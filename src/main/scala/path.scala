...
sealed trait ZKPathBase[S] {
   ...
   *
   *  '''Note''': Will capture any exception and wrap it into an Uncaught failure
   *
   *  @return `ZKState[S,A]`
   */
  def makePromise[A](f: PromisedResult[A] => Unit): ZKState1[A] = {
    try {
      ...
      case t: Throwable => stateT[PromisedResult, S, A](s => promise(uncaught(t).fail[(S, A)])(Strategy.Sequential))
    }
  }
  ...
  def stat(watch: Boolean = true): ZKState1[Stat] = {
    try {
      ...
      case t: Throwable => stateT[PromisedResult, S, Stat](s => promise(uncaught(t).fail[(S, Stat)])(Strategy.Sequential))
    }
  }
  ...
  def exists(watch: Boolean = true)(a: => ZKState1[Unit])(b: => ZKState1[Unit]): ZKState1[Unit] = {
    try {
      ...
      case t: Throwable => stateT[PromisedResult, S, Unit](s => promise(uncaught(t).fail[(S, Unit)])(Strategy.Sequential))
    }
  }
  ...
  /** Get the stat and ACL of a node.
   *
   *  @return `ZKState[S,(Stat,Seq[ZKAccessControlEntry]]]`
   */
...
class ZKPathWriter[S](val path: String, val connection: ZK) extends ZKPathBase[S] {
   ...
   *  @param acl A `Seq` of `ZKAccessControlEntry` values.
  ...
  /** Create a new node at the current path only if the node does not exist, otherwise do nothing
   *
   *  @tparam T The type of data to retrieve.  There is a context bounds of `ZKSerialize` on this type
   *   so an instance of `ZKSerialize` for the desired type has to be visible in implicit scope.
   *  @param data The data to be written (serialized) into the node.  Of type T
  ...
  /** Update the data of the node at the current path.
   *
   *  @tparam T The type of data to retrieve.  There is a context bounds of `ZKSerialize` on this type
   *   so an instance of `ZKSerialize` for the desired type has to be visible in implicit scope.
   *  @param data The data to be written (serialized) into the node.  Of type T
  ...
  /** Update the ACL of the node at the current path.
   *
   *  @param acl A `Seq` of `ZKAccessControlEntry` values.
   *  @param version a value of type ZKVersion
   *  @return `ZKState[S,Stat]`
